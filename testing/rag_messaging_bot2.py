...
def handle_messages(client, message):
    ...
    if current_state == ChatState.START_M:
        ...
        if any(phrase in message_text.lower() for phrase in ['War das eine gute Antwort?'.lower()]):
            # ACTION
        ...
        elif 'Es wurden keine Dokumente für das angefragte'.lower() in message_text:
