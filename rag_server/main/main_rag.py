...
import openai
from rag_retrieval import retrieve_context,get_dates_documents,build_context,get_data_ids
from user_manager import add_user_to_db,is_user_allowed,get_allowed_users,users_table_schema,USER_DB_TABLE
from dotenv import load_dotenv
import pandas as pd
from database_manager import MySQLDB,ChromaDBConnector,PineconeDBConnector,PineconeDBConnectorHybrid
...
import threading
import json
from chat import ChatManager,messages_table_schema,prompts_table_schema,MESSAGES_TABLE,PROMPTS_TABLE
...
index_pinecone = 'rag-jina-embedding-sparse'
...
TABLE_NAME = 'embedding_table_pinecone'
    
sql_db_connector = MySQLDB(CONFIG_SQL_DB,DB_NAME)

# Create the Users table if it doesn't exist
sql_db_connector.create_table(USER_DB_TABLE, users_table_schema)
sql_db_connector.create_table(PROMPTS_TABLE, prompts_table_schema)
sql_db_connector.create_table(MESSAGES_TABLE, messages_table_schema)
...
def preprocess_user_query(user_chat,query):
    ...
    return user_chat.date_project,prompts,keyterms
...
No additional context was found for this question."""
        ...
        2. Mention the source of your answer. Don't mention the ID, but the name of the source. For example: Richtlinie BEG EM,  Technische FAQ BEG, etc.
        3. Return the answer, and the list of IDs of the source used to answer the question.
...
def callback_query(call):
    ...
    if call.data == "cb_yes":
        ...
        if context:  # Check if context is not NULL or empty
            ...
            records = get_data_ids(sql_db_connector,TABLE_NAME,context_list_ids)
            records = build_context(records,sql_db_connector,TABLE_NAME)
...
def echo_all(message,user_chat):
    ...
    if user_chat.id in user_send_pwd:
        ...
        date_project,prompts,keyterms = preprocess_user_query(user_chat,query=input_text)
        ...
        if dates_files == None:
            ...
            results,ids_retrieved,filtered_ids = retrieve_context(vector_db_connector=pine_cone_connector,
                                    ...
                                    distance_threshold_query = DIST_THRESHOLD_QUERY,
                                    gen_prompts = prompts,
                                    keyterms = keyterms)
